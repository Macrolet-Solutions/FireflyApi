...
3. Subscribe to registration request topic.
4. Load enabled Firefly devices from the database.
5. Start one actor per enabled device.
6. Each actor loads its slots and uses `current_task_id` if available; otherwise it waits for registration or initializes on keepalive according to the protocol behavior.
7. Serve FastAPI routes and the React static frontend.
...
- Configurable API authentication mode.
- API key authentication for public integration endpoints.
- Admin authentication for frontend/admin endpoints.
- Secrets loaded from environment variables or a local secrets file excluded from source control.
...
```text
FIREFLY_DATABASE_URL=sqlite:///./data/firefly.db
FIREFLY_MQTT_HOST=localhost
FIREFLY_MQTT_PORT=1883
FIREFLY_MQTT_USERNAME=
FIREFLY_MQTT_PASSWORD=
FIREFLY_MQTT_USE_TLS=false
FIREFLY_MQTT_CLIENT_ID=firefly-api-service
FIREFLY_MQTT_PROTOCOL_VERSION=1
FIREFLY_ACK_TIMEOUT_MS=7000
FIREFLY_PUBLIC_API_KEY=
FIREFLY_ADMIN_AUTH_MODE=none|basic|oidc
FIREFLY_EVENT_RETENTION_DAYS=30
```
